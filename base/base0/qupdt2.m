function qnb1 = qupdt2(qnb0, rv_ib, rv_in, f)
...
%             f - quaternion different order approximation(0, 1, 2, 4) % modify by
%                 myself -- Moxianmin
...
%         rv_ib0 = 1-n2*(1/8-n2/384); s = 1/2-n2*(1/48-n2/3840); 
        % 上面是原计算代码，下面的是根据四元数更新取不同阶数的近似
        if f == 1
            rv_ib0 = 1; s = 1/2;
        elseif f == 2
            rv_ib0 = 1-n2*1/8; s = 1/2;
        elseif f == 4
            rv_ib0 = 1-n2*(1/8-n2/384); s = 1/2-n2*1/48;
        elseif f == 0
            rv_ib0 = 1-n2*(1/8-n2/384); s = 1/2-n2*(1/48-n2/3840);
        end
    else
...
%         上面是原计算代码，下面的是根据四元数更新取不同阶数的近似
        if f == 1
            rv_in0 = 1; s = -1/2;
        elseif f == 2
            rv_in0 = 1-n2*1/8; s = -1/2;
        elseif f == 4
            rv_in0 = 1-n2*(1/8-n2/384); s = -1/2-n2*1/48;
        elseif f == 0
            rv_in0 = 1-n2*(1/8-n2/384); s = -1/2-n2*(1/48-n2/3840);
        end
    else
