function [avp, avp_1_total, avp_2_total] = inspure0(imu, avp0, href, isfig)
...
global glv
    [nn, ts, nts] = nnts(1, imu(2,end)-imu(1,end)); % 第一个参数为2，即nn==2，代表了姿态更新周期为imu采样频率的两倍，即0.2s
...
%     vp_fix = 'b';  % unsing baro height
    ...
    avp_1_total = zeros(fix(len/nn), 10);
    avp_2_total = zeros(fix(len/nn), 10);
    for k=1:nn:len-nn+1 % 姿态更新周期为imu采样频率的nn倍
        k1 = k+nn-1;
        wvm = imu(k:k1, 1:6);  t = imu(k1,end);
        [ins, avp_1, avp_2] = insupdate0(ins, wvm);  ins.eth.dgnt=t; % ins.vn(3) = 0;
        ...
        avp(ki,:) = [ins.avp; t]';
        avp_1_total(ki,:) = [avp_1; t]';% 检查变量
        avp_2_total(ki,:) = [avp_2; t]';% 检查变量
        ki = timebar;
    end
