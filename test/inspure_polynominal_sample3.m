function avp = inspure_polynominal_sample3(imu, avp0, href, isfig)
...
global glv
    [nn, ts, nts] = nnts(3, imu(2,end)-imu(1,end)); % 第一个参数为2，即nn==2，代表了姿态更新周期为imu采样频率的两倍，即0.2s
...
%     vp_fix = 'b';  % unsing baro height
    ...
    for k=1:nn:len-nn+1 % 姿态更新周期为imu采样频率的3倍
        k1 = k+nn-1;
        wvm = imu(k:k1, 1:6);  t = imu(k1,end);
        ins = insupdate_polynominal_sample3(ins, wvm);  ins.eth.dgnt=t; % ins.vn(3) = 0;
